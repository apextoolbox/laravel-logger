...
class LoggerMiddleware
{
    ...
    {
        if (!Config::get('apextoolbox.token')) {
            return false;
        }

        if (!Config::get('apextoolbox.enabled', true)) {
            return false;
        }

        $path = $request->path();
        $includes = Config::get('apextoolbox.path_filters.include', ['api/*']);
        $excludes = Config::get('apextoolbox.path_filters.exclude', []);
