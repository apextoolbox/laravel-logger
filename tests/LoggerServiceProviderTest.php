...
class LoggerServiceProviderTest extends TestCase
{
    ...
    {
        $this->assertNotNull(Config::get('apextoolbox'));
        $this->assertArrayHasKey('enabled', Config::get('apextoolbox'));
        $this->assertArrayHasKey('token', Config::get('apextoolbox'));
    }
    ...
    public function test_config_is_published_correctly()
    {
        ...
        $this->assertStringContainsString('apextoolbox.php', array_keys($publishedPaths)[0] ?? '');
    }
